      ...
      server.route([
        ...
        },
        {
          ...
          handler: async (request, h) => {
            // Feature flag for CDP uploaderenable-cdp-uploader
            const enableCdpUploader = config.get('enableCdpUploader')

            if (!enableCdpUploader) {
              // Existing logic (execute if flag is false)
              ...
              // Placeholder for CDP uploader logic (execute if flag is true)
              // TODO: Implement CDP uploader logic here
              return h.view('upload/index', {
                isAuthenticated: true,
                user: request.auth.credentials.user,
                status: 'info',
                message:
                  'CDP uploader is enabled. Custom logic not yet implemented.',
                model: request.query.model || 'model1',
                analysisType: request.payload?.analysisType || 'green'
              })
            }
          }
        },
